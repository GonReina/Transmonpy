...
delta=0 #change Energies so this is really the detuning
de=-1
wq=2*np.pi*5
EJ=2*np.pi*12
eta=.1
...
for ct in ct_mat:
    ...
    for i in tqdm(range(tsteps), desc=f"Simulating ct={ct:.5f}"):
    ...
    filename = "Vib_res_kappa_{:.3f}_ct_{:.5f}.npy".format(kappa, ct)

    np.save(filename, njt)
