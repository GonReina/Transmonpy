...
phia=0.019
kappa= 0.002
ct_mat=2*np.pi*np.linspace(-0.025, 0.05,80)[39:-1]
...
for ct in ct_mat:
    ...
    for i in tqdm(range(tsteps), desc=f"Simulating ct={ct:.3f}"):
    ...
    filename = "TransmonSim_kappa_{:.3f}_ct_{:.3f}.npy".format(kappa, ct)
